...
// each call to input() gets its own full timeout window (matching pyserial's
// per-read() timeout semantics), and the very first empty read is a hard
// failure rather than something to keep tolerating. A stray byte before the
// 0xC0 sync marker, or a bad escape sequence, is a FatalError in both — not
...
class SlipDecoder(private val input: () -> ByteArray, private val trace: (String) -> Unit) {
    ...
    fun readPacket(): ByteArray {
        var partial: ByteArrayOutputStream? = null
        var inEscape = false
        while (true) {
            val buffer = input()
            if (buffer.isEmpty()) {
                // input() already waited up to its own timeout and got nothing –
            ...
            trace("Read ${buffer.size} bytes: ${HexFormatter(buffer)}")
            for (b in buffer) {
                if (partial == null) {
                    if (b == 0xC0.toByte()) {
                        partial = ByteArrayOutputStream()
                    } else {
                        trace("Read invalid data: ${HexFormatter(buffer)}")
                    ...
                    if (inEscape) {
                        ...
                        when (b) {
                            ...
                            0xDD.toByte() -> partial!!.write(0xDB)
                                ...
                                trace("Read invalid data: ${HexFormatter(buffer)}")
                                throw FatalError("Invalid SLIP escape (0xdb, 0x%s)".format(hexify(byteArrayOf(b))))
                            }
                        }
...
open class ESPLoader {
    ...
    fun flushInput() { port.flushInput() }
