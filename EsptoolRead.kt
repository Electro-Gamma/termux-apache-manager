...
// ============================================================================
// Config constants (fallback only)
// ============================================================================
...
open class ESPLoader(
    ...
    // ---- public properties (mirror Python attributes) ----
    ...
    fun connect(mode: String = "default-reset", attempts: Int = DEFAULT_CONNECT_ATTEMPTS, detecting: Boolean = false, warnings: Boolean = true) {
        ...
        if (!detecting) {
            ...
            // This is a stub; in practice we instantiate the correct chip class.
            // We'll skip the full detection loop here as the caller already chose the chip.
        }
    }
    ...
    fun getUsbVidPid(): Pair<Int, Int>? {
        ...
        for (p in ports) {
            ...
            // jSerialComm 2.9.0 uses `getVendorID()` and `getProductID()` (note capitalization)
            val vid = try { p.vendorID } catch (e: Exception) { -1 }
            val pid = try { p.productID } catch (e: Exception) { -1 }
            if (vid > 0 && pid > 0) {
                cache["usb_vid"] = vid
                cache["usb_pid"] = pid
                return vid to pid
    ...
    private fun readEfuse(n: Int): Long = readReg(0x3FF5A000L + 4L * n)
...
val chipDefinitions: Map<String, ChipInfo> = mapOf(
    ...
    "esp32c61" to ChipInfo(
        ...
        imageChipId = 20,
        usesMagicValue = false,
        spiRegBase = 0x60003000L,  // same as C6
        spiUsrOffs = 0x18, spiUsr1Offs = 0x1C, spiUsr2Offs = 0x20,
        spiMosiDlenOffs = 0x24L, spiMisoDlenOffs = 0x28L,
        spiW0Offs = 0x58,
        spiAddrRegMsb = false,
...
// Stub loader classes – each is just a wrapper that sets IS_STUB = true
...
// ============================================================================
// Command-line helpers (same as before)
// ============================================================================
...
fun detectChip(port: String = ESPLoader.DEFAULT_PORT, baud: Int = ESPLoader.ESP_ROM_BAUD,
    ...
    // Try each chip definition in order
    // We'll create a temporary loader to send the sync and read the chip magic/ID
    // but we need to know the chip type first. Use the same detection as Python:
    // 1) Try get-security-info to get chip ID (works on newer chips)
    // 2) Fallback to reading magic value at CHIP_DETECT_MAGIC_REG_ADDR
    // We'll do it by trying each chip class one by one.

    // First, try all chips that support get-security-info (no magic value)
    for ((_, info) in chipDefinitions.filter { !it.second.usesMagicValue }) {
        try {
    ...
    // Fallback: try magic value chips
    for ((_, info) in chipDefinitions.filter { it.second.usesMagicValue }) {
        try {
            val esp = ESPLoader(info, port, baud, traceEnabled)
            esp.connect(connectMode, connectAttempts, detecting = true)
            val magic = esp.readReg(ESPLoader.CHIP_DETECT_MAGIC_REG_ADDR)
...
fun main(args: Array<String>) {
    try {
        ...
        if (baud > ESPLoader.ESP_ROM_BAUD) esp.changeBaud(baud)
