...
class SlipReader(private val port: SerialPort, private val trace: (String) -> Unit) : Iterator<ByteArray> {
    ...
    private fun readFromPort(): ByteArray {
        ...
        val bytesRead = port.readBytes(buffer, toRead.toLong())
        return if (bytesRead > 0) buffer.sliceArray(0 until bytesRead.toInt()) else byteArrayOf()
    }
}
...
open class ESPLoader(
    ...
    protected var _cache = mutableMapOf<String, Any?>(
        "flash_id" to null,
        "usb_vid" to null,
        "usb_pid" to null,
    ...
    open fun command(
        ...
        try {
            if (op != null) {
                ...
                trace(
                    "--- Cmd $opName (0x${op.toString(16).padStart(2, '0')}) | data_len ${data.size} | wait_response ${if (waitResponse) 1 else 0} | timeout ${timeout}ms ---",
                    newline = true
                )
            ...
            repeat(100) {
                ...
                if (respData.isNotEmpty() && respData[0].toInt() != 0 && respData.getOrNull(1)?.toInt() == ROM_INVALID_RECV_MSG) {
                    Thread.sleep(200)
                    val drainBuf = ByteArray(14 * 8)
                    serialPort?.readBytes(drainBuf, (14 * 8).toLong())
                    flushInput()
    ...
    open fun checkCommand(
        ...
        if (respData.size < respDataLen + statusBytesLength) {
            val statusBytes = if (respData.isNotEmpty()) respData.sliceArray(0 until minOf(statusBytesLength, respData.size)) else byteArrayOf(0, 0)
        ...
        val statusBytes = respData.sliceArray(respDataLen until minOf(respDataLen + statusBytesLength, respData.size))
        if (statusBytes.isNotEmpty() && statusBytes[0].toInt() != 0) {
            throw FatalError("Failed to $opDescription")
        }
        return if (respDataLen > 0) respData.sliceArray(0 until minOf(respDataLen, respData.size)) else byteArrayOf()
    }
    ...
    open fun getUsbVidPid(): Pair<Int?, Int?> {
        if (_cache["usb_vid"] != null && _cache["usb_pid"] != null) {
            return Pair(_cache["usb_vid"] as Int?, _cache["usb_pid"] as Int?)
        }
    ...
    open fun writeReg(addr: Int, value: Int, mask: Int = 0xFFFFFFFF.toInt(), delayUs: Int = 0, delayAfterUs: Int = 0) {
        var buf = ByteBuffer.allocate(16).order(ByteOrder.LITTLE_ENDIAN).apply {
            putInt(addr)
            putInt(value)
            putInt(mask)
            putInt(delayUs)
        ...
        if (delayAfterUs > 0) {
            buf += ByteBuffer.allocate(16).order(ByteOrder.LITTLE_ENDIAN).apply {
            ...
            try {
                if (mode != "no-reset-no-sync") {
                    ...
                    if (waiting > 0) {
                        ...
                        serialPort?.readBytes(readBytes, waiting.toLong())
                    }
                }
                flushInput()
                serialPort?.doWhatever()
                sync()
                lastError = null
                return
            } catch (e: Exception) {
    ...
    open fun flashId(cache: Boolean = true): Int {
        if (!cache || _cache["flash_id"] == null) {
            val spiFlashRdid = 0x9F
            _cache["flash_id"] = runSpiflashCommand(spiFlashRdid, byteArrayOf(), 24)
        }
    ...
    open fun getSecurityInfo(cache: Boolean = true): Map<String, Any?> {
        if (cache && _cache["security_info"] != null) {
            return _cache["security_info"] as Map<String, Any?>
        }
...
class ESP32StubLoader(val romLoader: ESP32ROM) : ESP32ROM(
    ...
    init {
        ...
        _cache.putAll(romLoader._cache)
    }
}
...
// Mock function to avoid errors
fun SerialPort.doWhatever() {}